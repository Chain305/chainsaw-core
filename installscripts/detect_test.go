...
func TestNPMDependencyMutation(t *testing.T) {
	manifest := []byte(`{"name":"patchy","version":"1.0.0","scripts":{"postinstall":"mv ./node_modules/fetch-page-assets/index.ts ./node_modules/fetch-page-assets/index.ts.bak || true && node ./scripts/patch-fetch-page-assets.mjs"}}`)
	got := NPM(manifest)
	if !got.HasInstallScript {
		t.Fatalf("dependency-mutation npm: HasInstallScript must be true, got %+v", got)
	}
	if got.InstallScriptFetchesRemote {
		t.Fatalf("dependency-mutation npm must not be fetches_remote, got %+v", got)
	}
	if got.Kind != KindMutatesDependency {
		t.Fatalf("Kind: got %q, want %q", got.Kind, KindMutatesDependency)
	}
	if ast := NPMAST(manifest); ast.Kind != KindMutatesDependency {
		t.Fatalf("NPMAST Kind: got %q, want %q", ast.Kind, KindMutatesDependency)
	}
}

func TestNPMKnownPatchToolDoesNotDependencyMutation(t *testing.T) {
	got := NPM([]byte(`{"name":"patched","version":"1.0.0","scripts":{"postinstall":"patch-package"}}`))
	if got.Kind == KindMutatesDependency || got.InstallScriptFetchesRemote {
		t.Fatalf("patch-package wrapper must stay warning-neutral beyond present, got %+v", got)
	}
}
