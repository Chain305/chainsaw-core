...
func registerCoreProvidersInOrder() {
	...
	// CORE: pysource — Python import-time malicious behavior (PyPI). Order 41
	// is the first slot past the existing core/premium range.
	RegisterProvider(ProviderRegistration{
		Name: "pysource", Tier: TierCore, Order: 41,
		Factory: func(BootstrapConfig) Provider { return newPysourceProvider() },
	})
	// CORE: iocscan — embedded indicators of compromise (cross-ecosystem).
	RegisterProvider(ProviderRegistration{
