...
func MergeScan(dst *ArtifactScanSection, src ArtifactScanSection) {
	...
	if src.ImportTimeExecution {
		...
		dst.ImportTimeDetail = src.ImportTimeDetail
	}
	if src.MaliciousIOC {
	...
	// reading. Convention (B10): 0 = "no age signal available" — the
	// producer default and this merge sentinel. Only strictly-positive
	// readings are merged, so an absent/unreachable signal stays 0 and
	// the policy matcher's `<= 0` guard fails open. NOTE: this means a
	// genuine day-0 account is also read as "no signal"; migrating to a
	// real -1 sentinel end-to-end is deferred (see the
	// MaintainerAccountAgeDays doc in core/policy/evaluator.go).
