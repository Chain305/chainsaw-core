...
func TestArtifactURLFor_GoRejectsNonModuleVersions(t *testing.T) {
	// NOT "1.8.1" — that is what a stored Go version actually looks like,
	// because the depparsers strip the leading v. Rejecting it was the defect
	// that made the first version of this change a no-op; it is now normalised
	// and accepted. What must still be refused is a non-semver sentinel.
	for _, ver := range []string{"", "latest", "main", "v", "not-a-version"} {
...
// The stored form — no leading v — must resolve.
func TestArtifactURLFor_GoNormalisesTheStrippedVPrefix(t *testing.T) {
	got, _ := artifactURLFor("go", "github.com/gorilla/mux", "1.8.1")
	if got != "https://proxy.golang.org/github.com/gorilla/mux/@v/v1.8.1.zip" {
		t.Errorf("stored version 1.8.1 produced %q", got)
	}
}
