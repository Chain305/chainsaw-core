...
// TestHuggingFaceSigstoreReportsIdentityOnly asserts the unparseable-bundle
// path: when a model.sig sidecar is present but its bytes are not a valid
// Sigstore bundle, we fail cleanly with StatusFailed + an "inspect bundle"
// error rather than a misleading digest-mismatch error or a false-positive
// verified.
func TestHuggingFaceSigstoreReportsIdentityOnly(t *testing.T) {
	// A deliberately malformed bundle — BundleSubjects should fail cleanly
	// with StatusFailed and a parse-error message before any Verify() call,
	// and must not return a digest-mismatch error.
