...
way to present this. Of the **46 policy conditions**, each ecosystem
...
prose, and a drift test asserts it matches the published matrix. It is queryable
...
## The 46 policy conditions

```
Scorecard                           MalwareIndex                        EPSS
CVE                                 PackageAge                          Cooldown
License                             HasProvenance                       Typosquat
CVSS                                ReservedNamespaces                  HasInstallScript
InstallScriptFetchesRemote          ImportTimeExecution                 MaliciousIOC
BuildRsExecutes                     PublisherChanged                    VersionAnomaly
HasHiddenUnicode                    PublishVelocityAnomaly              LicenseCopyleft
LicenseNonPermissive                LicenseExceptionPresent             LicenseAmbiguousClassifier
LicenseUnidentified                 DeprecatedByMaintainer              ShrinkwrapPresent
ManifestConfusion                   GitDependency                       HTTPTarballDependency
WildcardDependencyRange             BadDependencySemver                 UsesEval
NetworkAccess                       ShellAccess                         FilesystemAccess
EnvVarAccess                        NativeBinaryPresent                 HighEntropyStrings
URLStrings                          MinifiedCode                        TrivialPackage
TooManyFiles                        NonExistentAuthor                   FirstTimeCollaborator
SuspiciousRepoStars
```
