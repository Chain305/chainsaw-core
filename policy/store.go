...
type Conditions struct {
	...
	PackageAge     *int     `json:"packageAge,omitempty" yaml:"packageAge,omitempty"`         // days since the PACKAGE was first published, nil=any
	CooldownDays   *int     `json:"cooldownDays,omitempty" yaml:"cooldownDays,omitempty"`     // match if THIS VERSION was published within the last N days — publish-age quarantine; catches account-takeover / zero-hour novel versions of established packages; nil=off
...
// IsMonitorMode reports whether this rule is in monitor mode — it fires and
// is logged as a would-have-blocked candidate, but does NOT block the
// install. This is the per-rule enforce-vs-monitor flag WS4 #2 promises: the
// dashboard uses it to render "you're already detecting this; flip it to
// enforce" nudges, and the policy.mode.changed transition event keys off a
// change in this predicate. ModeBlockAfterGrace is NOT monitor — it is a
// block with a bounded downgrade window (and only when the grace flag is on),
// so it must never be counted as a monitor-mode rule here.
func (p Policy) IsMonitorMode() bool {
	return p.Mode == ModeMonitor
}
...
func hasPolicyCondition(conditions Conditions) bool {
	...
	if conditions.CooldownDays != nil {
		return true
	}
	...
	if conditions.ImportTimeExecution != nil {
		return true
	}
	...
	if conditions.MaintainerAccountAgeDaysMax != nil {
		return true
	}
	// SLSA-substrate ecosystem narrowing + attestation requirements —
	// the Tier-1 baseline system policy uses these as its only
	// constraints, so without recognising them validatePolicy rejects
	// every system policy at boot. The evaluator (evaluateAttestation)
	// gates on each of these independently, so any one alone is a valid
	// standalone policy.
	...
	if conditions.RequireSLSALevel != nil {
		return true
	}
	if hasMeaningfulValues(conditions.RequireBuilderID) ||
		hasMeaningfulValues(conditions.RequireBuilderIssuer) ||
		hasMeaningfulValues(conditions.RequireSourceRepo) {
		return true
	}
	if conditions.RequireTransparencyLog != nil || conditions.ForbidCacheStale != nil {
		return true
	}
	// AI artifact conditions. Each is gated independently by the
	// evaluator (matchesConditions tail), so any one alone is a valid
	// standalone policy.
	if conditions.DangerousPickle != nil || conditions.UnsafeSerializationFormat != nil ||
		conditions.ModelCardInjection != nil || conditions.AgentToolDangerousCapability != nil ||
		conditions.MCPServerDeclared != nil || conditions.PromptTemplateInjection != nil {
		return true
	}
	return false
}
