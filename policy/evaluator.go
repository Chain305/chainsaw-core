...
type EvaluationContext struct {
	...
	// PackageReleaseDate is when the PACKAGE was first published (creation).
	PackageReleaseDate *time.Time
	// VersionReleaseDate is when THIS VERSION was published — distinct from
	// PackageReleaseDate. Cooldown gates on this: a poisoned new version of an
	// old package (Axios-class account-takeover) has a young VersionReleaseDate
	// but an old PackageReleaseDate, so package-age would miss it.
	...
	// from "not applicable" (intelligence disabled, incomplete input),
	// which leaves it false. At the publish surface the Tier-1 pre-run
	// sets this so a repo policy can fail CLOSED on missing signals via
	// `input.signalsUnavailable == true`. Default-off: when no rule keys
	// on it, behaviour is unchanged (fail-open on the sparse context).
	...
	// merged Report. Convention: 0 = no age signal available (the
	// producer default and the merge sentinel — scanner.go keeps the
	// youngest reading that is > 0, so an absent/unreachable signal
	// stays 0). A MaintainerAccountAgeDaysMax condition guards with
	// `ctx.MaintainerAccountAgeDays <= 0` and stays inert (does not
	// fire) on that sentinel, so an unreachable upstream cannot silently
	// block traffic.
	//
	// KNOWN LIMITATION (B10): a genuine day-0 account is therefore
	// indistinguishable from "no signal" and also fails open. Migrating
	// to a real -1 sentinel end-to-end (producer, scanner merge,
	// matcher) would make age 0 actionable; deferred until the
	// (flag-off, wave-4) maintainer-age provider is turned on.
...
type Evaluator struct {
	...
	// metaMu guards the memoised policyMeta. meta caches the two
	// key-shaping booleans (scoped / grace) so the hot path doesn't
	// re-scan the policy list on every request; metaLoaded distinguishes
	// "computed, all-false" from "never computed". Both are reset under
	// the lock by InvalidateCache / InvalidateCacheForOrg so a policy
	// edit that flips a boolean is picked up with the same flush that
	// clears stale verdicts. metaOverride, when set, pins meta for tests
	// that exercise the cache path without a live store.
	metaMu       sync.RWMutex
	meta         policyMeta
	metaLoaded   bool
	metaOverride *policyMeta

	// listOverride, when set, replaces e.store.List() as the policy
	// source. Test-only seam so the store-backed Evaluate/cache path
	// (B1) can be exercised without a live Postgres — production always
	// leaves it nil and reads through the store.
	listOverride func() ([]Policy, error)
}
...
func (e *Evaluator) InvalidateCacheForOrg(orgID string) {
	...
	// policyMeta is not org-partitioned (this evaluator serves a single
	// org's store, or the meta is a whole-set summary), so a per-org
	// invalidation still forces a recompute on next use — cheap and
	// keeps the scoped/grace booleans honest after an edit.
	e.resetMeta()
}

// resetMeta drops the memoised policyMeta so the next Evaluate recomputes
// it from a fresh policy list. Called from the Invalidate path so the
// key-shaping booleans track policy edits. No-op when a test override is
// pinned. Safe on a nil evaluator.
func (e *Evaluator) resetMeta() {
	if e == nil {
		return
	}
	e.metaMu.Lock()
	if e.metaOverride == nil {
		e.metaLoaded = false
		e.meta = policyMeta{}
	}
	e.metaMu.Unlock()
...
// cachedMeta returns the memoised policyMeta and whether it is present.
// A pinned test override always wins.
func (e *Evaluator) cachedMeta() (policyMeta, bool) {
	e.metaMu.RLock()
	defer e.metaMu.RUnlock()
	if e.metaOverride != nil {
		return *e.metaOverride, true
	}
	return e.meta, e.metaLoaded
}

// storeMeta records a freshly computed policyMeta unless a test override
// is pinned.
func (e *Evaluator) storeMeta(m policyMeta) {
	e.metaMu.Lock()
	if e.metaOverride == nil {
		e.meta = m
		e.metaLoaded = true
...
// withPolicyMeta pins policyMeta for tests that drive Evaluate through a
// live store but want a deterministic key shape without re-deriving it.
// Test-only seam; production code lets metaFor compute it.
func (e *Evaluator) withPolicyMeta(m policyMeta) *Evaluator {
	if e == nil {
		return nil
	}
	e.metaMu.Lock()
...
func (e *Evaluator) Evaluate(ctx EvaluationContext, exceptionAgeDays int) (EvaluationResult, error) {
	if e == nil || (e.store == nil && e.listOverride == nil) {
		recordEvalResult("allow")
		return EvaluationResult{Action: ModeAllow, Reason: "no policy store"}, nil
	}

	// Determine the cache-key shape from the policy set (B1). meta is
	// memoised; when cold we must materialise it BEFORE building the key
	// on a cache hit, so we fetch the policy list up-front and reuse it
	// for the miss path below (no extra round trip). When meta is warm
	// we defer the List to the miss path, preserving the cache-hit fast
	// path that skips the store entirely.
	meta, metaReady := e.cachedMeta()
	var (
		policies    []Policy
		policiesSet bool
	)
	if !metaReady {
		fetched, err := e.listPolicies()
		if err != nil {
			recordEvalResult("error")
			return EvaluationResult{Action: ModeAllow, Reason: "policy fetch failed"}, err
		}
		meta = computePolicyMeta(fetched)
		e.storeMeta(meta)
		policies, policiesSet = fetched, true
	}

	key, keyable := evalCacheKeyFor(ctx, meta)
	...
	if !policiesSet {
		...
		// Refresh meta from this List so a policy edit that arrived
		// without an Invalidate (e.g. TTL-only wiring) still converges
		// the key shape within one miss.
		e.storeMeta(computePolicyMeta(fetched))
		policies = fetched
	}
...
// listPolicies returns the policy set, honouring a test-only
// listOverride when present and otherwise reading through the store.
func (e *Evaluator) listPolicies() ([]Policy, error) {
	if e.listOverride != nil {
		return e.listOverride()
	}
	return e.store.List()
}

// evalCacheKeyFor builds the cache key for ctx given the policy-set
// meta. Returns keyable=false when caching must be skipped:
//
//   - any load-bearing identifier (package name / version) is empty —
//     caching an "unknown package" decision would give a single stale
//     verdict to every miss-request that happens to lack a version; or
//   - the org has any grace (ModeBlockAfterGrace) policy — grace
//     resolution depends on `now` vs the window and per-request
//     SeenBefore, both of which a frozen cache entry cannot honour, so
//     grace orgs bypass the cache entirely (a hit-rate cost borne only
//     by flag-gated grace users).
//
// For orgs with scope-targeted policies the key is extended with a
// scope fingerprint (client / groups / IP / country) so two requesters
// with different scope identity never share a verdict (B1 leak fix).
// Orgs with only global, non-grace policies get the exact four-field
// key — byte-identical to the pre-B1 behaviour, no hit-rate regression.
func evalCacheKeyFor(ctx EvaluationContext, meta policyMeta) (cacheKey, bool) {
	if ctx.PackageName == "" || ctx.PackageVersion == "" {
		return cacheKey{}, false
	}
	if meta.hasGracePolicies {
		return cacheKey{}, false
	}
	k := cacheKey{
		OrgID:       ctx.OrgID,
	...
	if meta.hasScopedPolicies {
		k.Scope = scopeFingerprint(ctx.ClientID, ctx.ClientGroups, ctx.RequestingIP, ctx.RequestingCountry)
	}
	return k, true
}
...
// isEmpty reports whether a Scope targets nothing — i.e. "applies to
// all requesters". This is the single source of truth for the
// empty-scope convention shared by matchesScope (skip scope gating) and
// computePolicyMeta (an empty-scope policy does NOT make the org
// scope-aware, so it must not perturb the cache-key shape).
...
func matchesConditions(ctx EvaluationContext, cond Conditions) bool {
	...
	// Cooldown: match VERSIONS published within the last CooldownDays days
	// (publish-age quarantine). Uses VersionReleaseDate (this version's publish
	// date), NOT PackageReleaseDate — an account-takeover poisons a NEW version
	// of an OLD package (Axios/Chalk/Mastra), so the version is young but the
	// package is old; gating on package age would miss it entirely. Malicious
	// versions are typically unpublished within hours, so a short window defeats
	// the zero-hour class. Missing version date fails the match (fail-open: we
	// don't quarantine when we can't confirm the age, to avoid blocking on
	// absent metadata).
	if cond.CooldownDays != nil {
		if ctx.VersionReleaseDate == nil {
			return false
		}
		versionAge := int(time.Since(*ctx.VersionReleaseDate).Hours() / 24)
		if versionAge > *cond.CooldownDays {
			return false
		}
	}
	...
	if cond.ImportTimeExecution != nil {
		if *cond.ImportTimeExecution != ctx.ImportTimeExecution {
			return false
		}
	}
	...
	// the configured number of days old. Convention: 0 = no age signal
	// available (provider disabled or upstream unreachable); the guard
	// is `<= 0` and fails open so a transport blip cannot silently block
	// traffic. KNOWN LIMITATION (B10): a real day-0 account also reads
	// as 0 and thus fails open — see the MaintainerAccountAgeDays field
	// doc for the deferred -1-sentinel migration.
