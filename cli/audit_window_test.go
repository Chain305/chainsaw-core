...
// 10k-row / 90-day export ceiling cannot keep.
func TestAuditCopy_DoesNotClaimFullRange(t *testing.T) {
	...
	for _, want := range []string{"10000", "90"} {
		if !strings.Contains(auditExportWindowNote, want) {
			t.Errorf("export note should name the server's real window (%s): %q", want, auditExportWindowNote)
		}
	}
	if !strings.Contains(auditViewExportHint, "10000") {
		t.Errorf("the view hint should carry the export ceiling: %q", auditViewExportHint)
	}
}
