...
func runAuthClientCreate(cmd *cobra.Command, _ []string) error {
	...
	printRegistrationHint(out, resp.ConfigSnippets)
	return nil
}

// printRegistrationHint echoes a slug-correct .npmrc/pip.conf registration
// snippet so the CLI path is copy-paste-correct. The server's config_snippets
// carry the org's REAL slug in the registry path (repository/@<orgSlug>/...);
// a bare @default is correct ONLY for a fresh single-tenant self-host and on
// SaaS returns HTTP 400 (CHW-4314) so the firewall silently never fires.
//
// We prefer npm, then pip, then any remaining format so there's always a
// concrete, slug-correct line to paste. If the server returned no snippets
// (older proxy, or it couldn't render one), we point the operator at the
// dashboard — which always carries the real slug — rather than printing a
// misleading @default hint.
func printRegistrationHint(out io.Writer, snippets map[string]configSnippetItem) {
	snippet, ok := firstRegistrationSnippet(snippets)
	if !ok {
		fmt.Fprintln(out, "")
		fmt.Fprintln(out, "# Registration snippet: copy the ready-made config (with your org slug)")
		fmt.Fprintln(out, "# from the dashboard — Settings → Client Credentials → New.")
		fmt.Fprintln(out, "# Do NOT hand-write a placeholder slug on SaaS: the wrong slug")
		fmt.Fprintln(out, "# returns HTTP 400 (CHW-4314) and the firewall silently never fires.")
		return
	}
	fmt.Fprintln(out, "")
	if label := strings.TrimSpace(snippet.Filename); label != "" {
		fmt.Fprintf(out, "# Registration snippet (%s) — carries your org slug, ready to paste:\n", label)
	} else {
		fmt.Fprintln(out, "# Registration snippet — carries your org slug, ready to paste:")
	}
	fmt.Fprintln(out, strings.TrimRight(snippet.Content, "\n"))
}

// firstRegistrationSnippet picks a single representative snippet to echo,
// preferring the ecosystems most operators reach for first. Returns false
// when no usable snippet (non-empty Content) is available.
func firstRegistrationSnippet(snippets map[string]configSnippetItem) (configSnippetItem, bool) {
	for _, format := range []string{"npm", "npmjs", "pip", "pypi"} {
		if s, ok := snippets[format]; ok && strings.TrimSpace(s.Content) != "" {
			return s, true
		}
	}
	...
	return configSnippetItem{}, false
}
