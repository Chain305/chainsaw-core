...
func runSetup(cmd *cobra.Command, _ []string) error {
	...
	// Abandonment funnel: every step sets prog.Step before it can block or
	// fail. If runSetup unwinds before reaching the completed emit below,
	// the deferred guard reports WHERE the user dropped (server_url, auth,
	// org, persona, policy, summary_confirm) — so the funnel shows the
	// blocking step, not just "didn't finish". `completed` flips true only
	// on the successful save path, guaranteeing abandon XOR completed (one
	// or the other fires, never both).
	completed := false
	defer func() {
		if !completed && prog.Step != "" {
			cliEmit(telemetry.EventCLISetupAbandoned, map[string]any{"step": prog.Step})
		}
	}()
	...
	}
	{
		...
		// Resolve a pre-supplied token first so scripted / non-TTY setup can
		// succeed without the browser flow (which blocks ~5min headless) or
		// PromptPassword (which returns "" off a pipe). Precedence: --token
		// flag, then CHAINSAW_TOKEN env. We deliberately do NOT call cfgToken()
		// here — at this point viper may carry a stale config token, and the
		// flag + raw env are the only inputs that express the user's intent
		// for *this* run. Trim to match cargo_credentials.go / doctor_strict.go.
		resolvedToken, _ := cmd.Flags().GetString("token")
		resolvedToken = strings.TrimSpace(resolvedToken)
		if resolvedToken == "" {
			resolvedToken = strings.TrimSpace(os.Getenv("CHAINSAW_TOKEN"))
		}

		switch {
		case resolvedToken == "" && !stdinIsTerminal():
			// Non-interactive with nothing to authenticate with: fail fast
		...
		case resolvedToken != "":
			...
			// run the existing API-key validation path. Reuse validateToken
			// and the /api/auth/me probe so prog.OrgID is populated, letting
			// the org-selection step short-circuit just like the browser flow.
			token = resolvedToken
			...
			fmt.Printf("API key validated. Logged in as %s (role: %s)\n\n", me.Email, me.Role)

		default:
			// Interactive: no token resolved AND stdin is a TTY.
	...
	if !yes && !PromptConfirmDefaultYes("Save configuration?") {
		...
		// The deferred abandonment guard emits cli.setup.abandoned with
		// step="summary_confirm" on this return — no explicit emit here, or
	...
	completed = true
	cliEmit(telemetry.EventCLISetupCompleted, nil)
	return nil
}
