...
var policyCmd = &cobra.Command{
	Use:     "policy",
	GroupID: GrpPolicy, // POLICY & ENFORCEMENT (P2.11). Matches the foundation's
	// commandGroupByName map; set explicitly here so the policy surface's group
	// is pinned at definition time and survives any future map refactor.
...
func runPolicyDelete(cmd *cobra.Command, args []string) error {
	...
	if !yes {
		...
		// and exit 0, masking that the delete never happened. Require --yes
		// explicitly here and exit with a clear error instead.
		if !stdinIsTerminal() {
			return fmt.Errorf("refusing to delete policy %s without --yes (stdin is not a TTY, so there is no confirmation prompt to display). Re-run with --yes to confirm.", id)
		}
...
func runPolicySetStatus(status string) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		out := cmd.OutOrStdout()
		if useJSON(cmd) {
			return PrintJSONTo(cmd, resp.Policy)
		}
...
// stay forward-compatible. P2.11 (verdict envelopes).
const simulateSchemaVersion = "1.0"

type simulateResult struct {
	// SchemaVersion is the top-level envelope version (P2.11). First field so
	// it leads the JSON object; additive, so older consumers ignore it.
...
func runPolicySimulate(cmd *cobra.Command, args []string) error {
	...
	// conditions — i.e. the CLI's identifier-only match fully determined the
	// printed Outcome without the server evaluating CVSS/EPSS/etc. Used only
	// to qualify the human headline; result.Outcome stays machine-stable.
	...
	if asJSON {
		...
		return simulateExitError(result.Outcome)
	}
	...
	if identifierOnly {
		...
		// than over-claim a definitive block/allow. The --json Outcome above
		// is left unchanged for machine consumers.
		fmt.Fprintf(out, "Outcome:  %s (identifier-only match — runtime conditions not evaluated)\n", result.Outcome)
	} else {
...
// simulateExitError maps a simulate outcome to the process exit-code contract
// (exitcodes.go). A definitive enforcement verdict — "block" or "quarantine" —
// is the EXPECTED gate-failure outcome, so it returns ExitCodeError{ExitBlocked}
// AFTER the result has already been written (so `policy simulate ... ; echo $?`
// scripts and `--json` consumers still see the full envelope on stdout). Every
// other outcome ("allow", "monitor", "conditional", "no_match") is
// informational and returns nil → ExitOK.
//
// Err is left nil — mirroring the established auth.go ExitCodeError{Code:1}
// convention — because the verdict is already on stdout; the coded error only
// carries the exit code. Execute()'s renderError emits a terse "Error: exit 1"
// to stderr, which keeps the stdout JSON/text envelope byte-clean.
//
// Invariant (B): a block is distinguishable from an operational/tool error by
// exit code — ExitBlocked(1) vs ExitOpError(2) — and quiet/verbose never alter
// it. Outcome comparison is case-insensitive to tolerate any server-side casing.
func simulateExitError(outcome string) error {
...
func runPolicyFlipToBlock(cmd *cobra.Command, args []string) error {
	...
	yes, _ := cmd.Flags().GetBool("yes")
	if !yes {
		// Non-TTY callers never see the prompt — PromptConfirm returns false
		// and we'd silently print "Aborted." and exit 0, so the monitor
		// policy never flips. Require --yes explicitly and error instead.
		if !stdinIsTerminal() {
			return fmt.Errorf("refusing to flip policy %s to block without --yes (stdin is not a TTY, so there is no confirmation prompt to display). Re-run with --yes to confirm.", id)
		}
...
func runPolicyImport(cmd *cobra.Command, args []string) error {
	...
	// A fully-failed import (every policy rejected) must not look successful
