...
func runFindingList(cmd *cobra.Command, _ []string) error {
	...
	if len(resp.Findings) == 0 {
		fmt.Fprintln(cmd.OutOrStdout(), "No findings found.")
		if desc := describeFindingFilters(cmd); desc != "" {
...
// describeFindingFilters renders the active `finding list` filters as a
// compact, human-readable string (e.g. "status=new,acknowledged; severity=high")
// for the empty-result message, so an operator can immediately see *what* they
// filtered on rather than guessing whether the queue is genuinely empty. It
// reads the same flags buildFindingListQuery does and returns "" when no
// filter is set. Kept as a free function so it is unit-testable like its
// sibling, without a server round-trip.
func describeFindingFilters(cmd *cobra.Command) string {
	parts := []string{}
	if v, _ := cmd.Flags().GetStringSlice("status"); len(v) > 0 {
		parts = append(parts, "status="+strings.Join(v, ","))
	}
	if v, _ := cmd.Flags().GetStringSlice("severity"); len(v) > 0 {
		parts = append(parts, "severity="+strings.Join(v, ","))
	}
	if v, _ := cmd.Flags().GetString("policy-id"); v != "" {
		parts = append(parts, "policy-id="+v)
	}
	if v, _ := cmd.Flags().GetString("package"); v != "" {
		parts = append(parts, "package="+v)
	}
	if v, _ := cmd.Flags().GetString("assignee"); v != "" {
		parts = append(parts, "assignee="+v)
	}
	return strings.Join(parts, "; ")
}
...
func runFindingTransitionFactory(action, target string) func(*cobra.Command, []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		asJSON := useJSON(cmd)
		if asJSON {
			return PrintJSONTo(cmd, resp.Finding)
		}
		printSuccess(cmd.OutOrStdout(), cmd, fmt.Sprintf("Finding %s → %s", resp.Finding.ID, resp.Finding.Status))
...
func runFindingSnooze(cmd *cobra.Command, args []string) error {
	...
	printSuccess(cmd.OutOrStdout(), cmd, fmt.Sprintf("Finding %s snoozed until %s", resp.Finding.ID, until.UTC().Format(time.RFC3339)))
	return nil
}
...
func runFindingSuppress(cmd *cobra.Command, args []string) error {
	...
	printSuccess(cmd.OutOrStdout(), cmd, fmt.Sprintf("Finding %s suppressed (%s)", resp.Finding.ID, reason))
	return nil
}
...
func runFindingAssign(cmd *cobra.Command, args []string) error {
	...
	if user == "" {
		printSuccess(cmd.OutOrStdout(), cmd, fmt.Sprintf("Finding %s unassigned", resp.Finding.ID))
	} else {
		printSuccess(cmd.OutOrStdout(), cmd, fmt.Sprintf("Finding %s assigned to %s", resp.Finding.ID, user))
	}
	return nil
}
