...
func runDoctorVerifyHook(cmd *cobra.Command, args []string) error {
	...
	defer driveCancel()
	if !jsonMode {
		fmt.Fprintf(cmd.ErrOrStderr(), "verify-hook %s: driving synthetic install (up to 60s)...\n", manager)
	}
	...
	defer confirmCancel()
	if !jsonMode {
		fmt.Fprintf(cmd.ErrOrStderr(), "verify-hook %s: polling audit log for sentinel (up to %s)...\n", manager, timeout)
	}
...
func pollAuditReceipt(ctx context.Context, sentinel string) receiptResult {
	...
	// Audit API is reachable; poll until timeout or match. Track a
	// *trailing* run of transport failures so we can distinguish two
	// timeout cases at ctx.Done():
	//   - the API stayed reachable and genuinely never showed the
	//     sentinel → FAIL (unambiguous client bypass), and
	//   - the API became unreachable mid-poll and stayed that way →
	//     DEGRADED (we can no longer prove anything, so we must not
	//     emit the alarming BYPASS verdict on a flaky network).
	// consecutiveErr resets on any successful (non-matching) poll, so a
	// single late success still yields the genuine FAIL.
	...
	for {
		...
		case <-ticker.C:
			...
			if err := client.Get(path, &poll); err != nil {
				// Transient — keep polling, but remember it. A trailing
				// run of failures at timeout degrades rather than
				// reporting a false-positive bypass.
				lastPollErr = err
				consecutiveErr++
				continue
			}
			// Successful poll: clear the trailing-failure run so only
