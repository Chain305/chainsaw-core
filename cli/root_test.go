...
// text written. renderError writes directly to os.Stderr.
...
// TestRenderError_AuthHint asserts an auth-bucket error keeps its verbatim
// "Error:" line and gains the `chainsaw auth login` remediation hint.
func TestRenderError_AuthHint(t *testing.T) {
	got := captureStderr(t, func() {
		renderError(errors.New("server returned 401 unauthorized"))
	})
	if !strings.HasPrefix(got, "Error: server returned 401 unauthorized\n") {
		t.Fatalf("missing/altered Error line: %q", got)
	}
	if !strings.Contains(got, "chainsaw auth login") {
		t.Fatalf("auth error missing login hint: %q", got)
	}
}

// TestRenderError_NetworkHint asserts a network-bucket error gains the
// `chainsaw status` / --server remediation hint.
func TestRenderError_NetworkHint(t *testing.T) {
	got := captureStderr(t, func() {
		renderError(errors.New("dial tcp: connection refused"))
	})
	if !strings.HasPrefix(got, "Error: dial tcp: connection refused\n") {
		t.Fatalf("missing/altered Error line: %q", got)
	}
	if !strings.Contains(got, "chainsaw status") {
		t.Fatalf("network error missing status hint: %q", got)
	}
}

// TestRenderError_GenericNoHint asserts an unclassified error prints only the
// plain Error line with no trailing hint.
func TestRenderError_GenericNoHint(t *testing.T) {
	got := captureStderr(t, func() {
		renderError(errors.New("boom"))
	})
	if got != "Error: boom\n" {
		t.Fatalf("generic error should be a single line, got: %q", got)
	}
}

// TestRenderError_CHWEnvelopeUnchanged confirms the structured CHW- path is
// untouched: it renders code/message/reason/docs and emits no bucket hint.
func TestRenderError_CHWEnvelopeUnchanged(t *testing.T) {
	err := &apiError{
		Code:    "CHW-4314",
		Message: "legacy URLs without the org slug are disabled",
		Reason:  "missing org slug",
		Docs:    "https://docs.chain305.com/errors/CHW-4314",
	}
	got := captureStderr(t, func() { renderError(err) })
	if !strings.Contains(got, "Error CHW-4314:") {
		t.Fatalf("CHW envelope not rendered: %q", got)
	}
	if strings.Contains(got, "Hint:") {
		t.Fatalf("CHW envelope should not carry a bucket hint: %q", got)
	}
}

// TestClassifyCLIError_Buckets pins the classifier rows the renderError hint
// keys on, plus the new unknown-package-manager wording (which currently
// buckets as "other", documented in the plan).
func TestClassifyCLIError_Buckets(t *testing.T) {
	cases := map[string]string{
		"401 unauthorized":              "auth",
		"connection refused":            "network",
		"boom":                          "other",
		`unknown package manager "xyz"`: "other",
	}
	for msg, want := range cases {
		if got := classifyCLIError(fmt.Errorf("%s", msg)); got != want {
