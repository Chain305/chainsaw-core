...
func runPkgList(cmd *cobra.Command, _ []string) error {
	...
	if ecosystem != "" || repo == "" {
		...
		// The server can't filter by ecosystem yet, so we filter client-side
		// below. Fetching only `limit` rows would silently drop a matching
		// package that sorts past the limit before the filter runs; widen the
		// server window (capped) when --ecosystem is active, then truncate the
		// FILTERED set back to --limit.
		fetchLimit := limit
		if ecosystem != "" {
			fetchLimit = limit * 20
			if fetchLimit > 1000 {
		...
		// Apply the user's --limit to the post-filter set (the server window was
		// widened above to avoid dropping matches before filtering).
...
func runPkgInfo(cmd *cobra.Command, args []string) error {
	...
	if err := client.Get("/api/sbom?"+sbomQ.Encode(), &sbomResp); err != nil {
		// Don't fail the whole `pkg info` on a license-lookup miss, but don't
		// swallow it silently either — otherwise a missing "License:" line is
		// indistinguishable from "no license recorded" vs "the fetch failed".
		fmt.Fprintf(cmd.ErrOrStderr(), "warning: could not fetch license info: %v\n", err)
	} else {
