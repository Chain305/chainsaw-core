...
// ansiEscapeRE matches the SGR colour sequences doctor emits, for tests that
// need to compare visible layout independent of colour.
var ansiEscapeRE = regexp.MustCompile("\x1b\\[[0-9;]*m")

// TestDoctor_ColorDoesNotShiftColumns guards the table-alignment fix: colouring
// "yes" must not change column widths. Previously the table went through
// text/tabwriter, which counts ANSI escape bytes as visible width and so
// collapsed the INSTALLED/WIRED columns whenever a cell was coloured. The fix
// pads against the plain text; this test fails if that regresses.
func TestDoctor_ColorDoesNotShiftColumns(t *testing.T) {
	withHookEnv(t)
	// This test drives the color path via the stdoutIsTerminal seam, so
	// NO_COLOR must be ABSENT (present-empty is an opt-out per no-color.org
	// that would force the no-color branch regardless of the seam).
	os.Unsetenv("NO_COLOR")

	render := func(color bool) string {
		prev := stdoutIsTerminal
		stdoutIsTerminal = func() bool { return color }
		defer func() { stdoutIsTerminal = prev }()
	...
	if !strings.Contains(colored, ansiGreen) {
		t.Skip("no coloured cells in this environment; alignment check is vacuous")
	}
	if stripped := ansiEscapeRE.ReplaceAllString(colored, ""); stripped != plain {
		t.Fatalf("colour changed table layout:\n--- coloured (ANSI stripped) ---\n%s\n--- plain ---\n%s", stripped, plain)
	}
}
