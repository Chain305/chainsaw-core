...
func runDoctorLogs(cmd *cobra.Command, _ []string) error {
	...
	if useStdin {
		read, kept, err := filterLogStream(os.Stdin, cmd.OutOrStdout(), filter)
		if err != nil {
			return err
		}
		return reportLogResult(cmd, read, kept, sevRaw, "stdin")
	}
	...
	// A "fetching…" cue so a slow/large window doesn't look hung before the
	// first filtered line appears. Stderr, so piped stdout stays clean.
	fmt.Fprintf(cmd.ErrOrStderr(), "doctor: fetching %s logs (--since=%s)…\n", deployment, since)
	...
	read, kept, ferr := filterLogStream(stdout, cmd.OutOrStdout(), filter)
	werr := kubectl.Wait()
	if ferr != nil {
		return ferr
	}
	...
	return reportLogResult(cmd, read, kept, sevRaw, deployment)
}

// reportLogResult disambiguates the two empty-output cases the filter can
// produce. read==0 means we couldn't read any logs at all (wrong target, empty
// window) — a real failure, so it returns a non-zero error. read>0 && kept==0
// means logs were read but nothing matched the severity filter — a genuine
// "all clear" signal, so it prints an explicit note (stderr) and succeeds,
// instead of printing nothing and looking identical to the failure case.
func reportLogResult(cmd *cobra.Command, read, kept int, sevRaw, source string) error {
	if read == 0 {
		return fmt.Errorf("no log lines read from %s — check --deployment/--namespace and that the pod has logs in the --since window", source)
	}
	if kept == 0 {
		fmt.Fprintf(cmd.ErrOrStderr(), "doctor: no lines at or above %q severity (%d line(s) scanned).\n", sevRaw, read)
	}
	return nil
}
