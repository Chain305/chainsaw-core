...
func initTelemetry() *telemetry.Client {
	telemetryOnce.Do(func() {
		...
		// Telemetry flows only to a configured server (set by `chainsaw setup` /
		// login, or a build-time DefaultServer). Local-only users with no server
		// emit nothing: the hosted ingest requires a Bearer token (anonymous
		// POSTs 401), so inventing a default endpoint just produces futile 401s.
		// The funnel's data moment is signup — once signed in the CLI has both a
		// server URL and a token, and guard events flow then. CHAINSAW_TELEMETRY_
		// ENDPOINT still overrides via telemetry.Endpoint for self-hosters/tests.
