...
func TestDiffManifest_CargoLock_AddAndUpgrade(t *testing.T) {
	base := []byte(`# This file is automatically @generated by Cargo.
version = 3
...
version = "0.2.147"
`)
	head := []byte(`# This file is automatically @generated by Cargo.
version = 3
...
`)
	...
	added, upgraded, err := diffManifest(kindCargoLock, "Cargo.lock", base, head)
	if err != nil {
		t.Fatalf("diffManifest: %v", err)
	}

	foundTokioAdded := false
	for _, e := range added {
		if e.Name == "tokio" && e.Version == "1.33.0" {
			foundTokioAdded = true
			if e.Ecosystem != "cargo" {
				t.Errorf("expected Ecosystem=cargo for tokio, got %q", e.Ecosystem)
			}
		}
	}
	if !foundTokioAdded {
		t.Errorf("expected tokio@1.33.0 in added; got added=%v", added)
	}

	foundSerdeUpgraded := false
	for _, e := range upgraded {
		if e.Name == "serde" && e.Version == "1.0.190" && e.PreviousVersion != nil && *e.PreviousVersion == "1.0.188" {
			foundSerdeUpgraded = true
			if e.Ecosystem != "cargo" {
				t.Errorf("expected Ecosystem=cargo for serde, got %q", e.Ecosystem)
			}
		}
	}
	if !foundSerdeUpgraded {
		t.Errorf("expected serde upgraded 1.0.188->1.0.190; got upgraded=%v", upgraded)
	}
}
